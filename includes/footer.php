<footer>
  <div class="container">
    <div style="margin-bottom:10px;">© <?= date("Y") ?> PincodeLocator.co.in</div>
    <div style="font-size:14px;opacity:.95;display:flex;justify-content:center;gap:14px;flex-wrap:wrap;">
      <a href="/about.php" style="color:#fff;">About</a>
      <a href="/contact.php" style="color:#fff;">Contact</a>
      <a href="/privacy-policy.php" style="color:#fff;">Privacy Policy</a>
      <a href="/terms.php" style="color:#fff;">Terms</a>
      <a href="/disclaimer.php" style="color:#fff;">Disclaimer</a>
    </div>
    <div style="margin-top:10px;font-size:13px;opacity:.9;">For corrections or support: support@pincodelocator.co.in</div>
  </div>
</footer>
</body>
</html>
