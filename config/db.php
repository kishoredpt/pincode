...
if ($user === '' || $db === '') {
    ...
    echo '<p>Hostinger quick fix: create <code>config/db.credentials.php</code> with keys <code>host,user,pass,name,port</code>.</p>';
    echo '<p>Credential file paths checked: <code>' . htmlspecialchars(implode(' | ', $checkedCredentialPaths), ENT_QUOTES, 'UTF-8') . '</code></p>';
    exit;
}
